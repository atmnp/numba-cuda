...
from numba.cuda.np import npdatetime_helpers, numpy_support
    ...
    def test_hash(self):
        ...
        if numpy_version >= (2, 2):
            with self.assertRaises(ValueError) as raises:
                f(TD(3))
            self.assertIn(
                "Can't hash generic timedelta64", str(raises.exception)
            )
...
class TestDatetimeArithmetic(TestCase):
    ...
    def test_add_sub_timedelta(self):
        ...
        # s + ...
        check(
            DT("2000-01-01T01:02:03Z"), TD(2, "h"), DT("2000-01-01T03:02:03Z")
        )
        check(
            DT("2000-01-01T01:02:03Z"),
            TD(2, "ms"),
    ...
    def test_datetime_difference(self):
        ...
        with self.silence_numpy_warnings():
            ...
            for a, b in itertools.product(dts, dts):
                if not npdatetime_helpers.same_kind(
                    value_unit(a), value_unit(b)
                ):
                    continue
                self.assertPreciseEqual(sub(a, b), a - b, (a, b))
    ...
    def test_comparisons(self):
        ...
        # with times
        check_eq(
            DT("2014-01-01T00:01:00Z", "s"), DT("2014-01-01T00:01Z", "m"), True
        )
        check_eq(
